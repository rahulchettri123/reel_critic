...
const getUserProfile = async (userIdentifier: string) => {
  try {
    // Check if it's a potential username or ID
    const isObjectId = /^[0-9a-fA-F]{24}$/.test(userIdentifier);
    const queryParam = isObjectId ? 'userId' : 'username';
    
    const response = await fetch(`/api/user/profile?${queryParam}=${userIdentifier}`, {
      credentials: "include"
    })
...
export default function ProfilePage() {
  ...
  const handleFollow = async () => {
    ...
    try {
      ...
      // Determine if the parameter is likely a username or an ID
      const isId = /^[0-9a-fA-F]{24}$/.test(userId);
      const payload = isId 
        ? { targetUserId: userId }
      ...
      if (response.ok) {
        ...
        toast({
          title: "Action failed",
          description: data.error || "Failed to follow/unfollow user",
          variant: "destructive"
        })
      }
    } catch (error) {
  ...
  const fetchMovieDetails = async (movieId: string) => {
    try {
      ...
      if (data.movie && data.movie.title) {
        ...
        return {
          id: movieId,
          title: `Movie ${movieId}`, // cleaner display
          poster: `/placeholder.svg?height=450&width=300&text=${encodeURIComponent(movieId)}`,
          year: 'Unknown',
  ...
  // First, let's add the function to load network data
  const loadNetworkData = async (type: 'followers' | 'following') => {
    ...
    try {
      // Use the profile._id for API calls instead of userId from URL params
      if (type === 'followers') {
        const followersRes = await fetch(`/api/user/followers?userId=${profile._id}`);
        const followersData = await followersRes.json();
        setFollowers(followersData.followers || []);
      } else {
        const followingRes = await fetch(`/api/user/following?userId=${profile._id}`);
                  ...
                  <div className="flex items-center gap-3">
                    <Popover>
                      <PopoverTrigger asChild>
                        <button 
                          ...
                          onClick={() => loadNetworkData('followers')}
                        >
                          <Users className="h-4 w-4" />
                          <span>{userStats.followersCount || 0}</span>
                        </button>
                      </PopoverTrigger>
                          ...
                          {loadingNetwork ? (
                            ...
                            <div className="max-h-[300px] overflow-y-auto space-y-2">
                              {followers.map(follower => (
                                <Link key={follower._id} href={`/profile/${follower.username}`} className="flex items-center gap-2 hover:bg-muted p-2 rounded-md">
                                  <Avatar className="h-8 w-8">
                                    <AvatarImage src={follower.avatar || "/placeholder.svg"} alt={follower.name || "User"} />
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <button 
                          className="flex items-center gap-1 text-sm hover:text-primary transition-colors"
                          onClick={() => loadNetworkData('following')}
                        >
                          <UserCheck className="h-4 w-4" />
                          <span>{userStats.followingCount || 0}</span>
                        </button>
                      </PopoverTrigger>
                      <PopoverContent className="p-0 w-[240px]" align="start">
                        <div className="p-4">
                          ...
                          {loadingNetwork ? (
                            ...
                            <div className="max-h-[300px] overflow-y-auto space-y-2">
                              {following.map(followed => (
                                <Link key={followed._id} href={`/profile/${followed.username}`} className="flex items-center gap-2 hover:bg-muted p-2 rounded-md">
                                  <Avatar className="h-8 w-8">
                                    <AvatarImage src={followed.avatar || "/placeholder.svg"} alt={followed.name || "User"} />
                                    <AvatarFallback>{followed.name?.charAt(0) || "?"}</AvatarFallback>
                                  </Avatar>
                                  <div>
                                    <p className="text-sm font-medium">{followed.name}</p>
